...
const posPercentageSevenDayAverageCalculator = (totalArr, posArr, outputObj, datesArr) => {
  let i = 6
  while (i < datesArr.length) {
    outputObj[datesArr[i]] = parseFloat(parseFloat(Math.trunc((posArr[datesArr[i]] + (posArr[datesArr[i-1]]) + (posArr[datesArr[i-2]]) + (posArr[datesArr[i-3]]) + (posArr[datesArr[i-4]]) + 
