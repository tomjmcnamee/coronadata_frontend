...
function fetchAllStatesData (numberOfDays, fromToDatesValue) {
  ...
  return function (dispatch) {
    ...
    dispatch({ type: "SET NEW DEATH", payload: []})
      ...
      .then(resp => resp.json())
      .then((response) => {
        let posPercentages = buildPercentageArrays(response.newTotal,response.newPositive, response.allDatesArr, "new-positivePercent", 0) 
        let hospPercentages = buildPercentageArrays(response.newPositive,response.newHospitalized, response.allDatesArr, "new-hospitalizedPercent", 0) 
        if (!!fromToDatesValue)  
        ...
        dispatch({ type: "SET NEW POSITIVE PERCENT", payload: posPercentages[0]})
        dispatch({ type: "SET NEW HOSPITALIZED PERCENT", payload: hospPercentages[0]})
